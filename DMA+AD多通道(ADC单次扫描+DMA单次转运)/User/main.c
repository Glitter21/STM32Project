...
int main(void)
{
    ...
    sprintf(display_str1, "%d%%", (uint16_t)(light_intensity * 100)); // 将输出值格式化为字符串
		...
		// 热敏传感器相关参数
		float R1 = 10000.0;           // 额定电阻值（单位：欧姆）
		float T1 = 25.0;              // 额定温度（摄氏度）
		float B = 3950.0;             // 热敏电阻的B值（K）
		float resistance = (R1 * (Vref - Voltage[2])) / Voltage[2];  // 转换为热敏电阻阻值（欧姆）
		float temperature = 1.0 / ((1.0 / (T1 + 273.15)) + (1.0 / B) * log(resistance / R1)) - 273.15;// 转换为温度值（摄氏度）
...
//    sprintf(temperatureStr, "%.1f", temperature); // 格式化温度字符串
		...
		OLED_ShowNum(3, 6, (uint16_t)temperature, 2); // 显示温度值（取整）
